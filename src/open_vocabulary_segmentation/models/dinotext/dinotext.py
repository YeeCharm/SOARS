...
class DINOText(nn.Module):
    ...
    #     # [修复] 防止传入错误的模板名称 (如 ViT-B/16)，强制修正为 imagenet
    #     try:
    #         templates = get_template(template_set)
    #     except ValueError:
    #         # print(f"[Warning] Template set '{template_set}' not found. Fallback to 'imagenet'.")
    #         templates = get_template("full")
        
    #     # [新增] 加载 LLM 描述
    #     descriptions_dict = {}
    #     if use_llm and desc_path is not None:
    #         if os.path.exists(desc_path):
    #             print(f"[DINOText] Loading LLM descriptions from: {desc_path}")
    #             with open(desc_path, 'r') as f:
    #                 data = json.load(f)
    #                 # 尝试自动获取内层字典 (兼容 {"LoveDADataset": {...}} 的结构)
    #                 if len(data) > 0:
    #                     descriptions_dict = list(data.values())[0]
    #         else:
    #             print(f"[Warning] Description path {desc_path} not found! Falling back to class names.")
    ...
    #     # [N, T_total, L]
    #     # build_text_embedding 会自动对 dim=1 取平均，所以这里直接 stack 即可
    #     tokens = torch.stack(tokens)
    ...
    def build_dataset_class_tokens(self, template_set, classnames, use_llm=False, desc_path=None):
        # 还原为纯净版，只处理原始类名
        # 注意：这里我们故意忽略 use_llm 和 desc_path，因为我们要在 generate_masks 里手动处理
        tokens = []
        try:
            templates = get_template(template_set)
        except ValueError:
        ...
        for classname in classnames:
            if 'bert' not in self.clip_model_name:
                tokens.append(clip.tokenize([t.format(classname) for t in templates]))
            else:
                tokens.append(self.tokenizer([t.format(classname) for t in templates], return_tensors='pt', padding='max_length')['input_ids'])
    ...
    def generate_masks(
        ...
        # -----------------------------------------------------------
        # 1. 主路推理 (Baseline) - 保证基础分
        # -----------------------------------------------------------
        # TTA 逻辑保留
        angles = [0, 90, 180, 270] 
        accum_mask = None
        
        # 这一步计算出的 text_emb 是纯净的原始类名特征 (由 validate_seg 传入)
        
        for angle in angles:
            if angle == 0: img_input = image
            else: img_input = TF.rotate(image, angle)
            
            # 基础推理
            mask_base, _ = self._inference_single(
                img_input, img_metas, text_emb, classnames, text_is_token, apply_pamr, background_func, lambda_bg
            )
            # mask_base shape: [B, 7, H, W] (LoveDA 7 classes)

            # -------------------------------------------------------
            # 2. 辅路推理 (LLM Boosting) - 仅在 0 度时计算一次特征，节省显存
            # -------------------------------------------------------
            # 我们需要在这里手动加载 LLM 描述并计算特征
            if hasattr(self, 'llm_boost_emb') and self.llm_boost_emb is not None:
                 # 使用预计算好的 LLM 特征
                 mask_llm, _ = self._inference_single(
                    img_input, img_metas, self.llm_boost_emb, 
                    self.llm_class_names, # 这是一个很长的列表 [desc1, desc2...]
                    text_is_token, apply_pamr, background_func, lambda_bg
                )
                 # mask_llm shape: [B, N_descriptions, H, W]
                 
                 # === 聚合逻辑 ===
                 # 将所有描述词的得分，归还给对应的父类
                 # self.llm_parent_indices 记录了每个描述词属于哪个父类 ID
                 
                 # 初始化一个和 mask_base 一样大的零张量
                 boost_map = torch.zeros_like(mask_base)
                 
                 # 对每个父类 (除了背景) 进行增强
                 for cls_idx in range(len(classnames)):
                     if classnames[cls_idx].lower() in ['background', 'bg', 'clutter']:
                         continue # 不增强背景
                         
                     # 找到该父类对应的所有 LLM 描述的索引
                     indices = (self.llm_parent_indices == cls_idx).nonzero(as_tuple=True)[0]
                     
                     if len(indices) > 0:
                         # 取出这些描述的预测图
                         sub_masks = mask_llm[:, indices, :, :]
                         # 取最大值 (Max-Pooling) 作为增强信号
                         max_val, _ = sub_masks.max(dim=1)
                         boost_map[:, cls_idx, :, :] = max_val
                 
                 # === 核心融合公式 ===
                 # Base + alpha * LLM
                 # alpha 取 0.5 左右比较安全
                 mask_base = mask_base + 0.3 * boost_map
            
            # -------------------------------------------------------

            if angle != 0:
                mask_base = TF.rotate(mask_base, -angle)
            
            if accum_mask is None: accum_mask = mask_base
            else: accum_mask += mask_base
    ...
    # 新添加的一个函数，把 JSON 里的描述词变成 Embedding 存起来。
    def prepare_llm_features(self, json_path, classnames, template_set="full"):
        """
        预计算 LLM 描述词的特征，存入 self.llm_boost_emb
        """
        import json
        ...
        # 获取模板
        try: templates = get_template(template_set)
        except: templates = get_template("full")

        for cls_id, name in enumerate(classnames):
            if name.lower() in ['background', 'bg', 'clutter']:
                continue # 跳过背景
            
            if name in desc_dict:
                descs = desc_dict[name]
                for d in descs:
                    # 生成 Token
                    prompts = [t.format(d) for t in templates]
                    if 'bert' not in self.clip_model_name:
                        token = clip.tokenize(prompts)
                    else:
                        token = self.tokenizer(prompts, return_tensors='pt', padding='max_length')['input_ids']
                    
                    llm_prompts.append(token)
                    parent_indices.append(cls_id)
                    flat_names.append(d)
        
        if len(llm_prompts) == 0: return

        # 生成 Embedding
        all_tokens = torch.stack(llm_prompts) # [N_total, T, L]
        self.llm_boost_emb = self.build_text_embedding(all_tokens) # [N_total, Dim]
        self.llm_parent_indices = torch.tensor(parent_indices, device=device)
        self.llm_class_names = flat_names
        print(f"[DINOText] LLM features prepared: {len(flat_names)} descriptions.")
