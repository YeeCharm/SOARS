...
class DINOText(nn.Module):
    ...
    #     """
    #     tokens = []
    #     templates = get_template(template_set) # 获取模板列表 (e.g., "a photo of a {}")
    ...
    #                 data = json.load(f)
    #                 # 假设 JSON 结构是 {'DatasetName': {'class': ['desc1', ...]}}
    #                 # 我们取第一个 value (即 LoveDADataset 下的字典)，或者你可以写死 dataset name
    #                 # 为了通用，这里尝试自动获取内层字典
    #                 if len(data) == 1:
    #                     descriptions_dict = list(data.values())[0]
    #                 else:
    #                     # 如果有多个数据集，尝试匹配当前数据集名称，或者默认取第一个
    #                     descriptions_dict = list(data.values())[0] 
    #         else:
    ...
    #             # 使用扩展描述 (e.g., ["asphalt road", "paved path", ...])
    #             texts_to_prompt = descriptions_dict[classname]
    #         else:
    #             # 回退到原始类名 (e.g., ["road"])
    #             texts_to_prompt = [classname]
            
    #         # 2. 生成所有 Prompt (Templates x Descriptions)
    #         # 例如: 5 个描述 * 80 个模板 = 400 个句子
    #         all_prompts = []
    ...
    #     # [N, T_total, L]
    #     # 注意：这里 T_total = num_templates * num_descriptions
    #     # build_text_embedding 函数会自动对 dim=1 取平均，所以不需要额外改动后续代码
    #     tokens = torch.stack(tokens)
