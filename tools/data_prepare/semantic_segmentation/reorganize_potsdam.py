...
def mkdir_or_exist(path: str):
    ...
    MMCV 1.x provided `mmcv.mkdir_or_exist`, but MMCV 2.x removed it.
    Using stdlib keeps this script version-agnostic.
    """
    os.makedirs(path, exist_ok=True)


def get_progress(total: int):
    """Return a progress updater with a `.update()` method.

    - Prefer `tqdm` if available.
    - Fall back to a minimal counter (prints occasionally).
    """
    try:
        ...
        # Simple fallback: mimic MMCV ProgressBar interface.
        class _SimpleProgress:
            def __init__(self, total_):
                self.total = max(int(total_), 1)
                self.n = 0

            def update(self, n: int = 1):
                self.n += n
                # Print every 5% or on completion to avoid too much spam.
                step = max(self.total // 20, 1)
...
def main():
    ...
    for zipp in zipp_list:
        with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
            ...
            prog_bar = get_progress(len(src_path_list))
            for i, src_path in enumerate(src_path_list):
                ...
                # tqdm accepts `update(1)`; our fallback accepts `update(1)` too.
                prog_bar.update(1)

            # Close tqdm to flush the last line.
