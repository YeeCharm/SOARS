...
def _mkdir_or_exist(path):
    """Create a directory if it doesn't exist.

    This script originally relied on `mmcv.mkdir_or_exist`, which is not
    available in some MMCV 2.x installations. Using the stdlib keeps the
    dataset conversion logic unchanged.
    """
...
def _progress_bar(total):
    ...
    `mmcv.ProgressBar` is not available in some MMCV 2.x builds, so we
    gracefully fall back to `tqdm` if installed, otherwise a no-op.
    """
    try:
...
def main():
    ...
    _mkdir_or_exist(osp.join(out_dir, 'img_dir', 'train'))
    _mkdir_or_exist(osp.join(out_dir, 'img_dir', 'val'))
    _mkdir_or_exist(osp.join(out_dir, 'ann_dir', 'train'))
    _mkdir_or_exist(osp.join(out_dir, 'ann_dir', 'val'))
    ...
    with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
        for zipp in zipp_list:
            ...
            prog_bar = _progress_bar(len(src_path_list))
            for i, src_path in enumerate(src_path_list):
                ...
                try:
                    ...
                    # Backward-compat for MMCV-style ProgressBar.update()
                    prog_bar.update()

            if hasattr(prog_bar, 'close'):
                prog_bar.close()
