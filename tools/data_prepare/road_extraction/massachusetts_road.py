...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Convert Massachusetts dataset to mmsegmentation format')
    parser.add_argument('dataset_path', help='Massachusetts folder path')
...
def main():
    ...
    if args.out_dir is None:
        out_dir = osp.join(dataset_path, 'cvt')
    else:
    ...
    for img_name in os.listdir(image_path):
        shutil.copy(osp.join(image_path, img_name), osp.join(out_dir, 'image_cvt', img_name))
