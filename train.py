...
def train_and_eval(config_file, train_dataset, val_dataset, texts=None, images=None, model_type='cls', test_set=None, optimizer="adam", weight_decay=0.05, scheduler='linear', warmup=0, name_pedix='', save_head_activations=None):
    ...
    file_suffix = ""
    if config['train'].get('save_best_model', False):
        # np.argmin 返回最小值的索引，+1 转换为 Epoch 数
        best_epoch = np.argmin(val_losses) + 1 
        min_loss = val_losses[best_epoch - 1]
        file_suffix = f"_best_epoch{best_epoch}"
        print(f"[-] Best model found at Epoch {best_epoch} (Val Loss: {min_loss:.4f})")

    save_name = f"{out_path}{file_suffix}.pth"
