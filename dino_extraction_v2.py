...
def run_dinov2_extraction(model_name, data_dir, ann_path, batch_size, resize_dim=518, crop_dim=518, out_path=None, 
    ...
    # fix: torch.cuda.is_available must be called
    ...
    for i in tqdm(range(n_batch)):
        ...
        for j in range(start, end):
            if 'jpg' in data['images'][j]:
                ...
                # 1) 优先使用本地绝对路径（file_name 或 coco_url）
                try:
                    if isinstance(file_name, str) and os.path.isabs(file_name) and os.path.exists(file_name):
                        pil_img = Image.open(file_name)
                    elif isinstance(coco_url, str) and os.path.isabs(coco_url) and os.path.exists(coco_url):
                        pil_img = Image.open(coco_url)
                    # 2) 再尝试 data_dir + file_name（适配自定义数据集目录）
                    elif isinstance(file_name, str) and data_dir is not None and os.path.exists(os.path.join(data_dir, file_name)):
                ...
                if pil_img is None:
                    # 3) Recap (远程 http)
                    if isinstance(file_name, str) and 'http' in file_name:
                        try:
                ...
                if pil_img is None:
                    # 4) 兼容原始 COCO 2014/2017 目录结构（保持向后兼容）
                    try:
                        if isinstance(file_name, str) and 'train' in file_name:
                            pil_img = Image.open(os.path.join(data_dir, f"train2014/{file_name}"))
                        elif isinstance(file_name, str) and 'val' in file_name:
                            pil_img = Image.open(os.path.join(data_dir, f"val2014/{file_name}"))
                        elif isinstance(file_name, str) and 'test' in file_name:
                            pil_img = Image.open(os.path.join(data_dir, f"test2014/{file_name}"))
                        elif isinstance(coco_url, str) and 'train' in coco_url:
                            pil_img = Image.open(os.path.join(data_dir, f"train2017/{file_name}"))
                        elif isinstance(coco_url, str) and 'val' in coco_url:
                ...
                if pil_img is None:
                    # 5) 仍无法加载：记录失败，使用占位图像
                    pil_img = Image.new("RGB", (224, 224))  # generate dummy image
                    failed_ids.append(j)
                    n_errors += 1
